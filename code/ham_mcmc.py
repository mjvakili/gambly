...
def mcmc_mpi(Nwalkers, Niters, Mr, style, prior_name = 'first_try'): 
    '''
    Parameters
    -----------
    - Nwalker : 
    ...
    - Mr :
        Luminosity threshold of the sample
    -style:
        abundance matching style of the catalog   
    '''
    #data and covariance matrix
    fake_obs_icov = Data.load_covariance(Mr, style)
    fake_obs = Data.load_data(Mr, style)
    ...
    chain_file_name = ''.join([util.mcmc_dir(),'mcmc_chain_Mr',str(Mr),'_style_',str(style),'.hdf5'])
...
if __name__=="__main__": 
    ...
    mcmc_mpi(Nwalkers, Niters, Mr, style)
