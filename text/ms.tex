...
Furthermore, we discuss how taking assembly bias into account in a more complex HOD model can improve our modeling of galaxy clustering in certain brightness limits. Then, we make a qualitative comparison between the levels of assembly bias in the mock catalogs created from our constraints on the decorated HOD model, and the \citealt{hw2013} mock catalogs created using the abundance matching technique that uses maximum circular velocity $V_{\rm max}$ as subhalo property. Our comparison shows the levels of assembly bias seen in the predictions of both models follow the same trend. That is assembly bias is more prominent in lower luminosity-threshold samples and its impact on galaxy clustering is only significant on large scales.
