...
\author{Mohammadjavad Vakili \altaffilmark{1,$\dagger$}, David W. Hogg, ChangHoon~Hahn, Unspecified contributors}
\altaffiltext{$\dagger$}{\url{mjvakili@nyu.edu}}
...
\caption{We demonstrate the posterior predictions for the expected occupation statistics of galaxies. The shaded regions mark the 68 $\%$ confidence region of the posterior predictions. The observables used in fitting are $\bar{n}$ and $\gmf$. $\bf{Top \; panel}$: posterior predictions for the expected number of centrals as a function of halos for the two population of halos: $type$-1 (high concentration, sown with blue) and $type$-2 (low concentration, shown with yellow) halos. }
