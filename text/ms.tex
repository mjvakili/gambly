...
We aim to constrain the parameters of the HOD model described in \ref{subsubsec:hod}, and the parameters of the assembly bias HOD model described in \ref{subsubsec:decorated}, given two sets of SDSS measurements described in \ref{sec:data}. We sample from the posterior probability distribution $p(\theta|d) \propto p(d|\theta).\pi(\theta)$. Here, $\theta$ denotes $\{ \log M_{\rm min}, \sigma_{\log M}, \log M_{0}, \alpha, \log M_{1} \}$ in the HOD modeling, and $\{ \log M_{\rm min}, \sigma_{\log M}, \log M_{0}, \alpha, \log M_{1}, \mathcal{A}_{\rm cen}, \mathcal{A}_{\rm sat}\}$ in the assembly bias HOD modeling. Furthermore, $d$ denotes the combination $[n_{g},g(N)]$ when the group multiplicity function is used, and $[n_{g}, w_{\rm{p}}(r_{\rm{p}})]$ when 2PCF is used.
...
Another important ingradient of our analysis is specification of the prior probabilities over the parameters of the two halo occupation models. For both model, we use uniform flat priors for all the parameters. 
...
\\ \hline 
  ...
  $\sigma_{\rm \log M}$ & & Uniform & &  [0.1, 0.7] \\
...
For sampling from the posterior probability, we use the affine-invariant ensemble MCMC sampler $\mathtt{emcee}$ (CITE[FORMAN-MACKEY]).
